...
Ext.onReady(function() {
    ...
    window.storeLastOperations = Ext.create('Ext.data.ArrayStore', {
        fields: [
           {name: 'company'},
   ...
   window.storeActiveOperations = Ext.create('Ext.data.ArrayStore', {
        fields: [
           {name: 'company'},
           {name: 'price',      type: 'float'},
           {name: 'change',     type: 'float'},
		...
		// principal store
		// cargando el xml
	  Ext.define('currencies',{
        extend: 'Ext.data.Model',
        fields: [
            // set up the fields mapping into the xml doc
            // The first needs mapping, the others are very basic
            {name: 'currency',  mapping:'@Symbol'},
						{name:'Sell', mapping: 'Bid'},
						{name:'Buy', mapping: 'Ask'},
						'High',
						'Low',
						'Direction',
						'Last'
		...
		window.store = Ext.create('Ext.data.Store', {
		    ...
		    proxy: {
		        ...
		        //url : 'http://rates.fxcm.com/RatesXML',
						url: '/datas/currencies.xml',
		        reader: new Ext.data.XmlReader( {
		            type: 'xml',
		            totalRecords: "Rates", // The element which contains the total dataset size (optional)
							  record: "Rate"												
		        })
		    }
		});
